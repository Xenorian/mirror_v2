...
from apps.board.models import Contributor
import requests
import parsel
import time
...
class ReposRequester(BaseRequester):
    ...
    def get_contributor(self, username: str, repo_name: str, is_save=False):
        ...
        while 1:
            url = f"https://api.github.com/repos/{username}/{repo_name}/contributors?page={i + 1}&per_page={30}"
            self._test()
            ...
            for j in range(len(contributor_dict)):
                ...
                try:
                    Contributor.objects.get(contributor_id=contributor_dict[j].get('id'),project="pytorch")
                except:
                    new_contributor = Contributor(contributor_id=contributor_dict[j].get('id'),
                                                  login=contributor_dict[j].get('login'), project="pytorch")
