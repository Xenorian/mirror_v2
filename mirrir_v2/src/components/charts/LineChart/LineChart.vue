...
onMounted(() => {
  ...
  let _rawData = [];
  _rawData[0]=data.val[0].commitActivity[0];
  ...
  while(1){
    ...
    for(let i=0;i<data.val.length;i++){
      ...
      if(data.val[i].commitActivity[counter[i]][2] === date_now){
        _rawData[_rawData.length]= JSON.parse(JSON.stringify(data.val[i].commitActivity[counter[i]]))
  ...
  //   for(let j=1;j<data.val[i].commitActivity.length;j++){
  //     _rawData[_rawData.length]= JSON.parse(JSON.stringify(data.val[i].commitActivity[j]))
