...
// compare the top
while(1){
  date_this_turn = data.val[0].issueActivity[counter[0]][2];
  choosen_val = 0;
  ...
  for(let i=0;i<data.val.length;i++){
    if(data.val[i].issueActivity[counter[i]][2] === date_now){
      chartdata[i].data.push(data.val[i].issueActivity[counter[i]][0])
      counter[i]++;
    }
  }
  ...
  for(i=0;i<data.val.length;i++){
    if(counter[i]!==data.val[i].issueActivity.length - 1){
      break;
    }
  }
